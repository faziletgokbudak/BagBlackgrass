...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--list_path', type=str, default='/mnt/yifan/data/blackgrass/blackgrass/data_table.txt',
                        help='Dataset list')
    parser.add_argument('-r', '--root_dir', type=str, default='/mnt/yifan/data/blackgrass/blackgrass/',
                        help='Dataset list')
    ...
    parser.add_argument('-t', '--background_t', type=int, default=0, help='Threshold for filtering background [15]')
    args = parser.parse_args()
    levels = tuple(args.magnifications)
    assert len(levels) <= 2, 'Only 1 or 2 magnifications are supported!'
    path_base = os.path.join(args.dataset)
    if len(levels) == 2:
        out_base = os.path.join(args.dataset, 'pyramid')
    else:
        out_base = os.path.join(args.dataset, 'single')
    all_slides = []
    for i in open(args.list_path):
        if 'tif' not in i:
            continue
        else:
            res = i.split()
            for ind in range(0, 5):
                all_slides.append(res[ind])
