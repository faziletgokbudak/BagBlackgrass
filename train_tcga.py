    ...
    with torch.no_grad():
        for i in range(len(test_df)):
            ...
            test_predictions.extend(
                [(0.0 * torch.sigmoid(max_prediction) + 1.0 * torch.sigmoid(bag_prediction)).squeeze().cpu().numpy()])
...
def main():
    ...
    parser.add_argument('--dataset', default='/mnt/yifan/data/blackgrass/features/', type=str, help='Dataset folder name')
    ...
    # if args.model == 'dsmil':
    #     state_dict_weights = torch.jit.load('init.pth')
    ...
    if args.dataset == 'TCGA-lung-default':
        ...
        bags_csv = os.path.join(args.dataset, "bags_all" + '.csv')
